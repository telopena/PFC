...
/*
		...
		*/
		for( size_t j = 0; j < (almacena.get_noses()).size(); j++ ){ 
			...
			rectangle(frame,Point((almacena.get_faces())[i].x+(almacena.get_noses())[j].x,(almacena.get_faces())[i].y+(dimensiones_cara.height)/4+(almacena.get_noses())[j].y),Point((almacena.get_faces())[i].x+(almacena.get_noses())[j].x+(almacena.get_noses())[j].width,(almacena.get_faces())[i].y+(dimensiones_cara.height)/4+(almacena.get_noses())[j].y+(almacena.get_noses())[j].height), Scalar( 255, 0, 0 ), 2, 8, 0 );		
			}
